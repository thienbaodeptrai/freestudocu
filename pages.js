// FreeStudocu - page loading and blur removal.
//
...
(function () {
    ...
    function forceEagerImg(img, candidates) {
        ...
        const attempts = +(img.dataset.fsForced || 0);
        if (attempts >= 3) return;
        img.dataset.fsForced = attempts + 1;
    ...
    function injectPageImage(pf, candidates) {
        ...
        if (pf.dataset.fsInjecting) return;
        if ((+(pf.dataset.fsFail || 0)) >= 3) return;
        pf.dataset.fsInjecting = '1';
        let i = 0;
        (function tryNext() {
            if (i >= candidates.length) {
                pf.dataset.fsInjecting = '';
                pf.dataset.fsFail = (+(pf.dataset.fsFail || 0)) + 1;
                return;
            }
            const url = candidates[i++];
            const img = new Image();
    ...
    function injectPageText(a, pf, pageNum) {
        const url = FS.pageTextUrl(a, pageNum);
        if (!url) return;
        if (pf.dataset.fsTextTried) return;
        pf.dataset.fsTextTried = '1';
        let request;
        try {
        ...
        request
            ...
            .then(function (html) {
                if (html && html.indexOf('<span') !== -1 && !FS.hasTextSpans(pf)) {
                    ...
                    const bg = FS.bgImageUrl(a, pageNum);
...
pages.markGatedPage = function (a, pf, pageNum) {
    ...
    pages.ensureAllPagesLoaded = function () {
        ...
        pfs.forEach(function (pf, idx) {
            const pageNum = idx + 1;
            const bgUrl = FS.bgImageUrl(a, pageNum);
            ...
            if (imgs.length > 1) {
                const ours = pf.querySelector('img[data-fs-injected-img]');
                const real = Array.prototype.find.call(imgs, function (im) {
                    return !im.dataset.fsInjectedImg && FS.imgLoaded(im);
                });
                if (ours && real) ours.remove();
            }

            if (FS.isTextGated(a, pageNum)) {
                gated.push(pageNum);
    ...
    pages.primeAllPages = function () {
        ...
        const hasBlank = pfs.some(function (pf) {
            return !FS.hasTextSpans(pf) && !FS.imgLoaded(pf.querySelector('img'));
        });
        ...
        if (pfs.length > limit) {
            FS.log('priming first ' + limit + ' of ' + pfs.length +
    ...
    pages.unblurImages = function () {
        document.querySelectorAll('.pf img').forEach(function (img) {
            if (img.dataset.fsUnblurred) return;
            const curSrc = img.getAttribute('src');
            const clearSrc = FS.deblurUrl(curSrc);
            if (clearSrc) {
                img.dataset.fsUnblurred = '1';
                img.removeAttribute('srcset');
                showImg(img);
            ...
            const clearData = FS.deblurUrl(img.getAttribute('data-src'));
            if (clearData) {
