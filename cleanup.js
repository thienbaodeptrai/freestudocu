...
// expressed as attributes on <html> (see FS.applySettingsToDocument) so the CSS
...
(function () {
    ...
    cleanup.removePremiumBadges = function () {
        FS.removeMatching(FS.SELECTORS.premiumBadges);
    ...
    cleanup.removeStudocuDownloadButtons = function () {
        if (!FS.settings.showDownloadButton) return;

        document.querySelectorAll(FS.SELECTORS.nativeDownloadButton).forEach(function (el) {
        ...
        const isNativeDownload = function (el) {
            ...
            return FS.SELECTORS.nativeDownloadWords.test(el.textContent.trim());
        };
    ...
    cleanup.replaceLogos = function () {
        if (!FS.settings.replaceLogo) return;
        FS.SELECTORS.logos.forEach(function (selector) {
            document.querySelectorAll(selector).forEach(function (logo) {
                if (logo.closest('.freestudocu-logo-replaced')) return;
                const wrapper = document.createElement('a');
                wrapper.className = 'freestudocu-logo-replaced';
                wrapper.href = 'https://github.com/danieltyukov/freestudocu';
                wrapper.target = '_blank';
                wrapper.rel = 'noopener';
                wrapper.title = 'FreeStudocu on GitHub';
                wrapper.appendChild(buildLogoIcon());
                const text = document.createElement('span');
                text.className = 'freestudocu-logo-text';
                const brand = document.createElement('span');
                brand.textContent = 'FREE';
                const accent = document.createElement('span');
                accent.className = 'freestudocu-logo-accent';
                accent.textContent = 'STUDOCU';
